...
def hf_classify_document(text):
    ...
    candidate_labels = [
        "CM Cell Petition",
        "Anna University Grievance",
        "RTI Petitions",
        "Legal Notices",
        "MOUs: Opinion, Draft",
        "Domestic Enquiry (Internal)",
        "Petition & Affidavit"
    ]
...
def hf_summarize(text):
    url = "https://router.huggingface.co/hf-inference/models/sshleifer/distilbart-cnn-12-6"
...
def search():
    ...
    uploaded_by = data.get("uploaded_by", "").strip()
    ...
    # ==== FIX: Safe embedding ====
    if query:
        embed_vec = local_embed(query)  # returns 768-dim vector
    else:
        embed_vec = [0.0] * 768  # zero vector to prevent pgvector error
    ...
    if role == "admin":
        base_query = """
            SELECT d.id, d.filename, d.summary, d.classification, d.created_at,
                   ...
                   CASE 
                     WHEN %s != '' THEN 1 - (d.embedding <=> %s::vector)
                     ELSE 1.0
                   END AS similarity
            FROM documents d
        ...
        base_query = """
            ...
            WHERE d.user_id = %s
        """
        params = [query, embed_vec, user_id]

    # ==== Apply Filters (Name, Semantic, Type) ====
    if classification:
